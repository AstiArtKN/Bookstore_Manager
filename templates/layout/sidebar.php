<?php
if(!defined('_KTHopLe'))
{
    die('Truy cập không hợp lệ');
}

if(!isLogin()){
    redirect('?module=auth&action=login');
}

$user_detail = getCurrentUserFromToken();

// Nếu không phải QTV hoặc NV thì cút ra store
if ($user_detail['quyenHanId'] !== 'QTV' && $user_detail['quyenHanId'] !== 'NV') {
    redirect('?module=store&action=index');
}

?>

<!DOCTYPE html>
<html lang="vi">

<head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <link rel="stylesheet" href="<?php echo _HOST_URL_TEMPLATES; ?>/assets/css/reset.css" />
    <!-- FONT -->
    <link rel="preconnect" href="https://fonts.googleapis.com" />
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin />
    <link
        href="https://fonts.googleapis.com/css2?family=Inter:ital,opsz,wght@0,14..32,100..900;1,14..32,100..900&display=swap"
        rel="stylesheet" />

    <!-- style css -->
    <link rel="stylesheet" href="<?php echo _HOST_URL_TEMPLATES; ?>/assets/css/style.css?ver=<?php rand(); ?>" />

    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/7.0.1/css/all.min.css" />

    <script src="https://cdn.jsdelivr.net/npm/chart.js"></script>


    <!-- <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.8/dist/css/bootstrap.min.css" rel="stylesheet"
        integrity="sha384-sRIl4kxILFvY47J16cr9ZwB07vP4J8+LH7qKQnuqkuIAvNWLzeN8tE5YBujZqJLB" crossorigin="anonymous"> -->

    <title><?php echo $data['title'];?></title>

    <script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
</head>

<body>
    <!-- side bar -->
    <div class="sidebar">
        <div class="sidebar__logo">
            <div class="logo-svg">
                <svg class="logo-svg-book" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
                    <path
                        d="M160 576L448 576C501 576 544 533 544 480L544 160C544 107 501 64 448 64L144 64C117.5 64 96 85.5 96 112L96 400C96 420.9 109.4 438.7 128 445.3L128 512C110.3 512 96 526.3 96 544C96 561.7 110.3 576 128 576L160 576zM448 448C465.7 448 480 462.3 480 480C480 497.7 465.7 512 448 512L192 512L192 448L448 448zM369.1 208.4C371.6 202.3 380.3 202.3 382.8 208.4L393.1 233.2L419.9 235.3C426.5 235.8 429.1 244 424.1 248.3L403.8 265.9L410 292C411.5 298.4 404.5 303.5 398.9 300.1L376 286.1L353.1 300.1C347.5 303.5 340.5 298.5 342 292L348.2 265.9L327.8 248.4C322.8 244.1 325.5 235.9 332 235.4L358.8 233.3L369.1 208.5zM196 256C196 187.5 251.5 132 320 132C343.8 132 366.1 138.7 385 150.4C389.8 153.3 391.9 159.2 390 164.5C388.1 169.8 382.9 173.1 377.3 172.5C374.2 172.2 371.1 172 367.9 172C321.5 172 283.9 209.6 283.9 256C283.9 302.4 321.5 340 367.9 340C371.1 340 374.2 339.8 377.3 339.5C382.9 338.9 388.1 342.2 390 347.5C391.9 352.8 389.7 358.7 385 361.6C366.1 373.3 343.8 380 320 380C251.5 380 196 324.5 196 256z" />
                </svg>
            </div>
            <p class="logo-title">Bookstore Management</p>
        </div>

        <!-- navigation -->
        <div class="nav">
            <ul class="nav__list">
                <li class="nav__item">
                    <div class="nav__icon-left">
                        <a href="?module=dashboard&action=index">
                            <svg class="nav__icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
                                <path fill="currentColor"
                                    d="M448 304L467.9 304C483.4 304 496 291.4 496 275.9C496 268.3 492.9 261 487.4 255.7L283.5 59.1C276.1 52 266.3 48 256 48C245.7 48 235.9 52 228.5 59.1L24.6 255.7C19.1 261 16 268.3 16 275.9C16 291.4 28.6 304 44.1 304L64 304L64 448C64 483.3 92.7 512 128 512L213.7 512C221.1 505.4 230.1 500.6 240 498L240 368C240 362.5 240.7 357.1 242 352L232 352C218.7 352 208 341.3 208 328L208 280C208 266.7 218.7 256 232 256L280 256C293.3 256 304 266.7 304 280L304 304L448 304zM352 416L512 416L512 544L352 544L352 416zM288 384L288 544L256 544C247.2 544 240 551.2 240 560C240 586.5 261.5 608 288 608L576 608C602.5 608 624 586.5 624 560C624 551.2 616.8 544 608 544L576 544L576 384C576 366.3 561.7 352 544 352L320 352C302.3 352 288 366.3 288 384z" />
                            </svg>
                        </a>
                    </div>
                    <a href="?module=dashboard&action=index" class="nav__link">Trang Chủ</a>
                </li>
                <li class="nav__item">
                    <div class="nav__icon-left">
                        <a href="?module=books&action=list">
                            <svg class="nav__icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
                                <path fill="currentColor"
                                    d="M192 576L512 576C529.7 576 544 561.7 544 544C544 526.3 529.7 512 512 512L512 445.3C530.6 438.7 544 420.9 544 400L544 112C544 85.5 522.5 64 496 64L192 64C139 64 96 107 96 160L96 480C96 533 139 576 192 576zM160 480C160 462.3 174.3 448 192 448L448 448L448 512L192 512C174.3 512 160 497.7 160 480zM370.1 163.2C372.7 160.6 376.8 160.1 380 162.1C412.1 182.1 433.4 217.7 433.4 258.3C433.4 320.9 382.7 371.6 320.1 371.6C257.5 371.6 206.7 320.9 206.7 258.3C206.7 217.7 228.1 182.1 260.1 162.1C263.2 160.1 267.3 160.6 270 163.2C272.7 165.8 273.1 169.9 271.2 173C266 181.6 263 191.7 263 202.5C263 217.6 268.9 231.3 278.5 241.6C281 244.3 281.4 248.3 279.4 251.3C274.7 258.7 272 267.4 272 276.8C272 298.4 286.3 316.7 306 322.7L307 297.9C299.9 293.5 295.2 285.7 295.2 276.8C295.2 267.2 300.7 258.8 308.7 254.7L312 172.9C312.2 168.6 315.7 165.2 320 165.2C324.3 165.2 327.8 168.6 328 172.9L331.3 254.7C339.3 258.8 344.8 267.1 344.8 276.8C344.8 285.7 340.1 293.5 333 297.9L334 322.7C353.6 316.7 367.9 298.6 368 277.1L368 276.5C367.9 267.2 365.3 258.6 360.6 251.3C358.7 248.2 359 244.2 361.5 241.6C371.1 231.4 377 217.7 377 202.5C377 191.7 374 181.6 368.8 173C366.9 169.8 367.4 165.8 370 163.2z" />
                            </svg>
                        </a>
                    </div>
                    <a href="?module=books&action=list" class="nav__link">Quản Lý Sách</a>
                </li>
                <li class="nav__item">
                    <div class="nav__icon-left">
                        <a href="?module=book_author&action=list">
                            <svg class="nav__icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
                                <path fill="currentColor"
                                    d="M267 48C230.6 48 209.2 106.3 198.7 160L168 160C154.7 160 144 170.7 144 184C144 197.3 154.7 208 168 208L192 208L192 240C192 257 195.3 273.2 201.3 288L192 288L192 288L171.5 288C156.3 288 144 300.3 144 315.5C144 318.5 144.5 321.4 145.4 324.2L174.3 410.8C136.2 443.6 112 492.1 112 546.3C112 562.7 125.3 576 141.7 576L498.3 576C514.7 576 528 562.7 528 546.3C528 492.1 503.8 443.6 465.7 410.9L494.6 324.3C495.5 321.5 496 318.6 496 315.6C496 300.4 483.7 288.1 468.5 288.1L448 288.1L448 288.1L438.7 288.1C444.7 273.3 448 257.1 448 240.1L448 208.1L472 208.1C485.3 208.1 496 197.4 496 184.1C496 170.8 485.3 160.1 472 160.1L441.3 160.1C430.9 106.4 409.4 48.1 373 48.1C363.4 48.1 354 52 345.5 56.3C337.3 60.4 327.1 64.1 320 64.1C312.9 64.1 302.7 60.4 294.5 56.3C286 51.9 276.6 48 267 48zM360.7 532.4L335.9 461.5L363.8 429C366.5 425.8 368 421.8 368 417.6C368 407.9 360.2 400.1 350.5 400.1L289.5 400.1C279.8 400.1 272 407.9 272 417.6C272 421.8 273.5 425.8 276.2 429L304.1 461.5L279.3 532.4L222.3 352L258 352C276.4 362.2 297.5 368 320 368C342.5 368 363.6 362.2 382 352L417.7 352L360.7 532.4zM320 320C285.3 320 255.8 297.9 244.7 267C250.4 270.2 257 272 264 272L276.4 272C292.9 272 307.5 261.4 312.7 245.8C315 238.8 324.9 238.8 327.2 245.8C332.4 261.4 347.1 272 363.5 272L375.9 272C382.9 272 389.5 270.2 395.2 267C384.1 297.9 354.6 320 319.9 320z" />
                            </svg>
                        </a>
                    </div>
                    <a href="?module=book_author&action=list" class="nav__link">Tác giả</a>
                </li>
                <li class="nav__item">
                    <div class="nav__icon-left">
                        <a href="?module=book_category&action=list">
                            <svg class="nav__icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
                                <path fill="currentColor"
                                    d="M96 160C96 124.7 124.7 96 160 96L480 96C515.3 96 544 124.7 544 160L544 480C544 515.3 515.3 544 480 544L160 544C124.7 544 96 515.3 96 480L96 160zM160 224L160 480L288 480L288 224L160 224zM480 224L352 224L352 480L480 480L480 224z" />
                            </svg>
                        </a>
                    </div>
                    <a href="?module=book_category&action=list" class="nav__link">Thể Loại</a>
                </li>
                <!-- <li class="nav__item">
                    <div class="nav__icon-left">
                        <a href="">
                            <svg class="nav__icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
                                <path fill="currentColor"
                                    d="M352 97.8L352 224L80.5 224C132.2 146.8 220.1 96 320 96C330.8 96 341.5 96.6 352 97.8zM400 224L400 107.3C466 126.3 522.2 168.3 559.5 224L400 224zM54.6 272L128 272L128 384L32 384C32 344.3 40 306.4 54.6 272zM208 384L176 384L176 272L464 272L464 384L432 384L432 432L608 432L608 496C608 522.5 586.5 544 560 544L80 544C53.5 544 32 522.5 32 496L32 432L208 432L208 384zM512 384L512 272L585.4 272C600 306.4 608 344.3 608 384L512 384zM320 352C284.7 352 256 380.7 256 416L256 496L384 496L384 416C384 380.7 355.3 352 320 352z" />
                            </svg>
                        </a>
                    </div>
                    <a href="" class="nav__link">NXB</a>
                </li> -->

                <li class="nav__item">
                    <div class="nav__icon-left">
                        <a href="?module=receipt&action=list">
                            <svg class="nav__icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
                                <path fill="currentColor"
                                    d="M544 72C544 58.7 533.3 48 520 48L418.2 48C404.9 48 394.2 58.7 394.2 72C394.2 85.3 404.9 96 418.2 96L462.1 96L350.8 207.3L255.7 125.8C246.7 118.1 233.5 118.1 224.5 125.8L112.5 221.8C102.4 230.4 101.3 245.6 109.9 255.6C118.5 265.6 133.7 266.8 143.7 258.2L240.1 175.6L336.5 258.2C346 266.4 360.2 265.8 369.1 256.9L496.1 129.9L496.1 173.8C496.1 187.1 506.8 197.8 520.1 197.8C533.4 197.8 544.1 187.1 544.1 173.8L544 72zM112 320C85.5 320 64 341.5 64 368L64 528C64 554.5 85.5 576 112 576L528 576C554.5 576 576 554.5 576 528L576 368C576 341.5 554.5 320 528 320L112 320zM159.3 376C155.9 396.1 140.1 412 119.9 415.4C115.5 416.1 111.9 412.5 111.9 408.1L111.9 376.1C111.9 371.7 115.5 368.1 119.9 368.1L151.9 368.1C156.3 368.1 160 371.7 159.2 376.1zM159.3 520.1C160 524.5 156.4 528.1 152 528.1L120 528.1C115.6 528.1 112 524.5 112 520.1L112 488.1C112 483.7 115.6 480 120 480.8C140.1 484.2 156 500 159.4 520.2zM520 480.7C524.4 480 528 483.6 528 488L528 520C528 524.4 524.4 528 520 528L488 528C483.6 528 479.9 524.4 480.7 520C484.1 499.9 499.9 484 520.1 480.6zM480.7 376C480 371.6 483.6 368 488 368L520 368C524.4 368 528 371.6 528 376L528 408C528 412.4 524.4 416.1 520 415.3C499.9 411.9 484 396.1 480.6 375.9zM256 448C256 412.7 284.7 384 320 384C355.3 384 384 412.7 384 448C384 483.3 355.3 512 320 512C284.7 512 256 483.3 256 448z" />
                            </svg>
                        </a>
                    </div>
                    <a href="?module=receipt&action=list" class="nav__link">Quản Lý Đơn Hàng</a>
                </li>
                <li class="nav__item">
                    <div class="nav__icon-left">
                        <a href="">
                            <svg class="nav__icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
                                <path fill="currentColor"
                                    d="M544.4 304L368.4 304C350.7 304 336.4 289.7 336.4 272L336.4 96C336.4 78.3 350.8 63.8 368.3 66.1C475.3 80.3 560.1 165.1 574.3 272.1C576.6 289.6 562.1 304 544.4 304zM254.6 101.2C272.7 97.4 288.4 112.2 288.4 130.7L288.4 328C288.4 333.6 290.4 339 293.9 343.3L426 502.7C437.7 516.8 435.2 538.1 419.1 546.8C385 565.4 345.9 576 304.4 576C171.9 576 64.4 468.5 64.4 336C64.4 220.5 145.9 124.1 254.6 101.2zM509.8 352L573.8 352C592.3 352 607.1 367.7 603.3 385.8C593.1 434.2 568.3 477.2 533.7 510C521.4 521.7 502.1 519.2 491.3 506.1L406.9 404.4C389.6 383.5 404.5 352 431.5 352L509.7 352z" />
                            </svg>
                        </a>
                    </div>
                    <a href="" class="nav__link">Thống kê - Báo Cáo</a>
                </li>
                <?php if($user_detail['quyenHanId'] === 'QTV'): ?>
                <li class="nav__item">
                    <div class="nav__icon-left">
                        <a href="?module=users&action=list">
                            <svg class="nav__icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
                                <path fill="currentColor"
                                    d="M136 192C136 125.7 189.7 72 256 72C322.3 72 376 125.7 376 192C376 258.3 322.3 312 256 312C189.7 312 136 258.3 136 192zM48 546.3C48 447.8 127.8 368 226.3 368L285.7 368C384.2 368 464 447.8 464 546.3C464 562.7 450.7 576 434.3 576L77.7 576C61.3 576 48 562.7 48 546.3zM544 160C557.3 160 568 170.7 568 184L568 232L616 232C629.3 232 640 242.7 640 256C640 269.3 629.3 280 616 280L568 280L568 328C568 341.3 557.3 352 544 352C530.7 352 520 341.3 520 328L520 280L472 280C458.7 280 448 269.3 448 256C448 242.7 458.7 232 472 232L520 232L520 184C520 170.7 530.7 160 544 160z" />
                            </svg>
                        </a>
                    </div>
                    <a href="?module=users&action=list" class="nav__link">Danh Sách Người Dùng</a>
                </li>
                <?php endif;?>
            </ul>
        </div>
        <!-- Account -->
        <div class="account">
            <h2 class="account__header">Trang Tài Khoản</h2>
            <ul class="account__list">
                <li class="account__item">
                    <div class="nav__icon-left">
                        <a href="">
                            <svg class="nav__icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
                                <path fill="currentColor"
                                    d="M192 64C156.7 64 128 92.7 128 128L128 512C128 547.3 156.7 576 192 576L448 576C483.3 576 512 547.3 512 512L512 128C512 92.7 483.3 64 448 64L192 64zM288 416L352 416C396.2 416 432 451.8 432 496C432 504.8 424.8 512 416 512L224 512C215.2 512 208 504.8 208 496C208 451.8 243.8 416 288 416zM264 320C264 289.1 289.1 264 320 264C350.9 264 376 289.1 376 320C376 350.9 350.9 376 320 376C289.1 376 264 350.9 264 320zM280 128L360 128C373.3 128 384 138.7 384 152C384 165.3 373.3 176 360 176L280 176C266.7 176 256 165.3 256 152C256 138.7 266.7 128 280 128z" />
                            </svg>
                        </a>
                    </div>
                    <a href="" class="account__link">Profile</a>
                </li>
                <!-- <li class="account__item">
                    <div class="nav__icon-left">
                        <a href="">
                            <svg class="nav__icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
                                <path fill="currentColor"
                                    d="M384 64C366.3 64 352 78.3 352 96C352 113.7 366.3 128 384 128L466.7 128L265.3 329.4C252.8 341.9 252.8 362.2 265.3 374.7C277.8 387.2 298.1 387.2 310.6 374.7L512 173.3L512 256C512 273.7 526.3 288 544 288C561.7 288 576 273.7 576 256L576 96C576 78.3 561.7 64 544 64L384 64zM144 160C99.8 160 64 195.8 64 240L64 496C64 540.2 99.8 576 144 576L400 576C444.2 576 480 540.2 480 496L480 416C480 398.3 465.7 384 448 384C430.3 384 416 398.3 416 416L416 496C416 504.8 408.8 512 400 512L144 512C135.2 512 128 504.8 128 496L128 240C128 231.2 135.2 224 144 224L224 224C241.7 224 256 209.7 256 192C256 174.3 241.7 160 224 160L144 160z" />
                            </svg>
                        </a>
                    </div>
                    <a href="" class="account__link">Sign In</a>
                </li> -->
                <!-- <li class="account__item">
                    <div class="nav__icon-left">
                        <a href="">
                            <svg class="nav__icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
                                <path fill="currentColor"
                                    d="M192 384L88.5 384C63.6 384 48.3 356.9 61.1 335.5L114 247.3C122.7 232.8 138.3 224 155.2 224L250.2 224C326.3 95.1 439.8 88.6 515.7 99.7C528.5 101.6 538.5 111.6 540.3 124.3C551.4 200.2 544.9 313.7 416 389.8L416 484.8C416 501.7 407.2 517.3 392.7 526L304.5 578.9C283.2 591.7 256 576.3 256 551.5L256 448C256 412.7 227.3 384 192 384L191.9 384zM464 224C464 197.5 442.5 176 416 176C389.5 176 368 197.5 368 224C368 250.5 389.5 272 416 272C442.5 272 464 250.5 464 224z" />
                            </svg>
                        </a>
                    </div>
                    <a href="" class="account__link">Sign Up</a>
                </li> -->
                <li class="account__item">
                    <div class="nav__icon-left">
                        <a href="">
                            <svg class="nav__icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
                                <path fill="currentColor"
                                    d="M192 384L88.5 384C63.6 384 48.3 356.9 61.1 335.5L114 247.3C122.7 232.8 138.3 224 155.2 224L250.2 224C326.3 95.1 439.8 88.6 515.7 99.7C528.5 101.6 538.5 111.6 540.3 124.3C551.4 200.2 544.9 313.7 416 389.8L416 484.8C416 501.7 407.2 517.3 392.7 526L304.5 578.9C283.2 591.7 256 576.3 256 551.5L256 448C256 412.7 227.3 384 192 384L191.9 384zM464 224C464 197.5 442.5 176 416 176C389.5 176 368 197.5 368 224C368 250.5 389.5 272 416 272C442.5 272 464 250.5 464 224z" />
                            </svg>
                        </a>
                    </div>
                    <a href="?module=auth&action=logout" class="account__link">Đăng xuất</a>
                </li>
            </ul>
        </div>
    </div>